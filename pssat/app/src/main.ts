...
export function createNav(nav_data: NavData): void {
  const nav = document.getElementById("nav") as HTMLUListElement;
  nav.innerHTML = "";
...
function createSlide(slide_data: SlideData): void {
  const slideContainer = document.getElementById("slide") as HTMLDivElement;
  ...
  slide_data.slides.forEach((slideItem, index) => {
    ...
    slideDiv.classList.add(
      ...
      "items-center",
      "justify-center",
      "bg-contain", // bg-cover yerine bg-contain
      "bg-center",
      "bg-no-repeat" // boşluk oluşmaması için tekrar etmesini engeller
    );
...
function changeSlide(wrapper: HTMLDivElement): void {
  currentSlideIndex = (currentSlideIndex + 1) % totalSlides;
  updateSlidePosition(wrapper);
}

function updateSlidePosition(wrapper: HTMLDivElement): void {
  wrapper.style.transform = `translateX(-${currentSlideIndex * 100}%)`;
}

function createInfo(info_data: InfoData): void {
  const info = document.getElementById("info") as HTMLDivElement;
  info.innerHTML = "";

  const infoDiv = document.createElement("div");
  infoDiv.classList.add(
    "flex",                  // İçerikleri esnek kutuda yerleştir
...
export function createProducts(prodacts_data: ProdactsData): void {
  ...
  products.classList.add(
    "grid", 
    "grid-cols-1", // küçük ekranda 1 kolon
...
export function createFooter(footer_data: FooterData): void {
  const footer = document.getElementById("footer") as HTMLDivElement;
  footer.innerHTML = "";
  
  footer.classList.add(
    "bg-blue-900",       // Mavi arka plan
  ...
  for (const branch of footer_data.branchs) {
    ...
    branchDiv.classList.add(
      "bg-blue-700",  // Şube kartı için biraz daha açık mavi
      "p-6",          // İçerik boşluğu
